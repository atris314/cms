...
//        $result = $this->send($api, $amount, $redirect);
        $result = $this->curl_post('https://nextpay.org/nx/gateway/token',
            'api_key='.$request->api_key.'&amount='.$amount.'&order_id='.$order->id.'&customer_phone='.$request->phone.
            '&custom_json_fields={ "product_id":'.$request->product_id.', "user_id":'.$request->user_id.' }&callback_uri='.$callback);
...
//                $paymentTrackid=$request->payment;
                ...
                elseif($ordernextpay->payer_desc=='پرداخت نهایی'){
                    ...
                    if ($ordernextpay->phone){
                        try {
                            $receptor = $ordernextpay->phone;
                            $type = 1;
                ...
                $msg='پرداخت تکمیل و با موفقیت انجام شده است .شماره پیگیری تراکنش'.$nextpayment->track_id;
//                return redirect()->route('paymentResult')->with('success',$msg);
            ...
            $msg = 'پرداخت لغو شد!' . $request->status;
            return redirect(route('oldproduct'))->with('danger',$msg);
        }
        else
            $msg = 'متاسفانه پرداخت با مشکل مواجه شد دوباره تلاش کنید! .شماره وضعیت' . $request->status;
            return redirect(route('oldproduct'))->with('danger',$msg);
    }
...
//        dd($payments);
    ...
    public function paymentShow(Nextpayment $nextpayment)
    {
//        dd($nextpayment);
        $user = Auth::user();
        $productcount = \App\frontmodels\Product::with('user')->with('message')
            ->where('user_id',$user->id)
            ->where('status','<>','15')->count();
        return view('front.dashboard.paymentShow',compact('nextpayment','user','productcount'));
    }
}
