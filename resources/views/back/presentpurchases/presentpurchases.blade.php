                ...
                <div class="box-body">
                    ...
                    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap"><div class="row"><div class="col-sm-6"></div><div class="col-sm-6"></div></div><div class="row"><div class="col-sm-12">
                                <table id="example2" class="table table-responsive table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                                    ...
                                    <tbody>
                                    @foreach($protransactions as $protransaction)
                                        <tr role="row" class="odd">
                                            <td class="text-center" dir="ltr">{{$protransaction->user->code}}</td>
                                            <td class="text-center">{{$protransaction->paid}} تومان </td>
                                            <td class="text-center">
                                                @if($protransaction['status']==0)
                                                    <a  class="label pull-center bg-maroon" >تراکنش ناموفق</a>
                                                @elseif($protransaction['status']==1)
                                                    <a  class="label pull-center bg-navy" >تراکنش نامشخص در حال انجام</a>
                                                @elseif($protransaction['status']==2)
                                                    <a  class="label pull-center bg-olive" >تراکنش موفق</a>
                                                @endif
                                            </td>
                                            <td class="text-center" style="font-size:10px;">{{\Hekmatinasser\Verta\Verta::instance($protransaction->created_at)}}</td>
                                            <td class="text-center" style="width: 180px;">
                                                <a class=" label pull-center bg-aqua" href="{{route('back.presentpurchases.show',$protransaction->id)}}">جزئیات تراکنش</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
