    ...
    <div class="row">
        <div class="col-xs-4">
            <div class="box" style="padding: 30px ">
                <form action="{{route('productstoreSearch')}}" method="get">
                    @csrf
...
{{--                            <input type="text" class="form-control" placeholder="عبارت مورد جستجو ..." name="title" value="{{old('title')}}" >--}}
        ...
        <div class="col-xs-4">
            <div class="box" style="padding: 30px ">
                <form action="{{route('productstoreCodeSearch')}}" method="get">
                    @csrf
        ...
        <div class="col-xs-4">
            <div class="box" style="padding: 30px ">
                <form action="{{route('sotreuserCodeSearch')}}" method="get">
                    @csrf
                        ...
                        <table id="example2" class="table table-responsive table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            ...
                            @foreach($products as $product)
                                <tr role="row" class="odd">
                                    ...
                                    <td class="text-center"><a href="{{route('back.productstore.show',$product->id)}}">{!! mb_substr($product->title,0,10).'..'!!} </a></td>


                                        @if(isset($product->user->code))
                                        ...
                                        @endif
                                        @if(Auth::user())
                                            <td>{{$product->user->rate}}</td>
                                            @elseif(Auth::user() && $product['status']==1)
                                            <td>{{$product->user->rate+=10}}</td>
                                            @elseif(Auth::user() && $product['status']==2)
                                            <td>{{$product->user->rate+=20}}</td>
                                            @elseif(Auth::user() && $product['status']==3)
                                            <td>{{$product->user->rate+=30}}</td>
                                            @elseif(Auth::user() && $product['status']==4)
                                            <td>{{$product->user->rate+=40}}</td>
                                            @else
                                            <td></td>
                                        @endif
                                    ...
                                    <td class="text-center">
                                        ...
                                        <td class="text-center">
                                        @if($product['status']==0)
                                            <a  class="label pull-center bg-maroon" >ثبت شده</a>
                                        @elseif($product['status']==1)
                                            <a  class="label pull-center bg-navy" >کوپن تخفیف خود را استفاده کرده</a>
                                        @elseif($product['status']==2)
                                            <a  class="label pull-center bg-olive" >پرداخت اولیه</a>
                                        ...
                                        <a class=" label pull-center bg-green" href="{{route('back.productstore.show',$product->id)}}">نمایش</a>
                                        <a class=" label pull-center bg-aqua" href="{{route('back.products.edit',$product->id)}}">ویرایش</a>
                                        <a class=" label pull-center bg-red" href="{{route('back.productstore.destroy',$product->id)}}" onclick="return confirm('آیتم مورد نظر حذف شود؟');">حذف</a>
                                    </td>
                                </tr>
                            @endforeach
                           </tbody>
